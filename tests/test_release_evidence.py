...
def test_private_artifact_snapshot_survives_caller_path_replacement(
    ...
    original = tmp_path / "fast_fsm-0.5.0-py3-none-any.whl"
...
def test_verify_wheel_classifies_universal_wheel_without_native_members(
    ...
    assert artifact["metadata_version"] == "0.5.0"
...
def test_slots_policy_authorities_name_the_same_four_exceptions() -> None:
    ...
    policy_sentence = (
        "CompiledFuncCondition, TransitionError, TransitionRejected, and "
        "DiagnosticBudgetExceeded"
    )
...
def _validate_tag_release_workflow(workflow: dict[str, object]) -> None:
    ...
    assert "--tag-ref v0.5.0" in tag_runs
    assert "aggregate_conclusion" in tag_runs
    assert "git rev-parse v0.5.0^{}" in tag_runs
...
def _release_asset_aggregate(payloads: Mapping[str, bytes]) -> dict[str, object]:
    ...
    direct_name = "fast_fsm-0.5.0-py3-none-any.whl"
    archive_name = "fast_fsm-0.5.0.tar.gz"
    child_name = "fast_fsm-0.5.0-derived.whl"
    return {
        "profile": "release",
...
def test_stage_release_assets_publishes_only_direct_and_sdist_archive(
    ...
    payloads = {
        "fast_fsm-0.5.0-py3-none-any.whl": b"pure-wheel",
        "fast_fsm-0.5.0.tar.gz": b"source-archive",
        "fast_fsm-0.5.0-derived.whl": b"derived-wheel",
    }
    downloaded = tmp_path / "downloaded"
    downloaded.mkdir()
    for filename in (
        "fast_fsm-0.5.0-py3-none-any.whl",
        "fast_fsm-0.5.0.tar.gz",
    ):
...
def test_stage_release_assets_rejects_unapproved_or_substituted_downloads(
    ...
    if mutation == "extra":
        ...
        (downloaded / "fast_fsm-0.5.0.tar.gz").write_bytes(b"substituted")
    elif mutation == "symlink":
        target = downloaded / "target.whl"
        target.write_bytes(payloads["fast_fsm-0.5.0-py3-none-any.whl"])
        (downloaded / "fast_fsm-0.5.0-py3-none-any.whl").unlink()
        (downloaded / "fast_fsm-0.5.0-py3-none-any.whl").symlink_to(target.name)
    else:
        (downloaded / "fast_fsm-0.5.0-derived.whl").write_bytes(
            payloads["fast_fsm-0.5.0-derived.whl"]
        )
...
def _matrix_artifact_name(cell: object) -> str:
    ...
    return f"fast_fsm-0.5.0{build_tag}-cp{minor}-cp{minor}-{platform_tag}.whl"
    ...
    *,
    ...
    record: dict[str, object] = {
        ...
        if cell.sdist_parent is None
            ...
            "filename": "fast_fsm-0.5.0.tar.gz",
            "sha256": hashlib.sha256(b"fast_fsm-0.5.0.tar.gz").hexdigest(),
        },
    }
    return record
...
def test_aggregate_matrix_accepts_reproducible_sdist_pure_wheel_reuse() -> None:
    ...
    filename = "fast_fsm-0.5.0-py3-none-any.whl"
...
def _phase20_baseline_static_fixture() -> dict[str, object]:
    """Return the generator's static v0.5.0 baseline envelope without measurements."""
    return {
        "schema_version": 2,
        "release_identity": {
            "package": "fast_fsm",
...
def _write_release_identity_fixture(
    ...
    """Create complete v0.5.0 static surfaces without creating a Git tag."""
    (root / "docs").mkdir()
    (root / "evidence").mkdir()
    ...
    (root / "CHANGELOG.md").write_text(
        f"# Changelog\n\n## [0.5.0] — {changelog_date}\n\nRelease proof.\n",
        encoding="utf-8",
    )
    (root / "README.md").write_text(
        "Fast FSM v0.5.0 provides installed-artifact release proof. "
...
def _identity_inputs() -> tuple[dict[str, str], dict[str, str]]:
    ...
    return (
        {"distribution_version": "0.5.0", "package_version": "0.5.0"},
        {
            "package": "fast_fsm",
            "distribution_version": "0.5.0",
            "commit": "a" * 40,
...
def test_static_release_identity_requires_every_v040_surface(tmp_path: Path) -> None:
    """PR-time validation accepts v0.5.0 without assuming its public tag exists."""
