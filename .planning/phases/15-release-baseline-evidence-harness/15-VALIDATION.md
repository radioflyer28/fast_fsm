...
| 15-08-01 | 08 | 5 | REL-05, REL-06 | T-15-32, T-15-33, T-15-34, T-15-35 | Every Taskfile-consuming CI job installs exact Task 3.53.1 through `arduino/setup-task@v3.0.0` before its first `task` invocation while all existing gate contracts remain unchanged | fail-first Task bootstrap and ordering contract added to `tests/test_release_evidence.py` inside this task | `uv run pytest tests/test_release_evidence.py -x -q -k "task_runner or workflow_contract or setup_uv or supported_python"`; YAML parse; full `tests/test_release_evidence.py`; pure source verification; `FAST_FSM_BUILD_MODE=pure task release-gate` | pending |
