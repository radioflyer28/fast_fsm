...
| 15-09-01 | 09 | 6 | REL-05, REL-06 | T-15-36, T-15-37 | Exact Python patches remain serialized while only stable comparison uses fail-closed major.minor identity; same-minor drift is fresh and different-minor/test-count/pin drift remains stale | fail-first portability, malformed-version, exact-count, coverage, and strict-pin cases added to `tests/test_release_evidence.py` | `uv run pytest tests/test_release_evidence.py -x -q -k "python and (toolchain or manifest or freshness)"`; full release-evidence tests; Ruff; mypy; pure source preflight | pending |
| 15-09-02 | 09 | 6 | REL-05, REL-06, TEST-02 | T-15-37, T-15-38, T-15-39 | A committed clean pure archive writes the exact current inventory (at least 782), then a second committed clean archive passes read-only freshness and the release gate without closing either CI bead | existing baseline collector plus Task 1 tests; no missing scaffold | pure locked sync -> immediate preflight -> `task release-baseline-write`; exact manifest assertions; second archive `task release-baseline-check`; `task release-gate` | pending |
| 15-05-01 | 05 | 7 | REL-05, REL-06 | T-15-26, T-15-27 | Corrective exact pushed SHA has terminal all-job proof including portable evidence freshness and locked sync/build for Python 3.10-3.14, then closes fast_fsm-6yg and fast_fsm-bhn | blocking external checkpoint; no local test scaffold can substitute for Actions | `gh run list --repo radioflyer28/fast_fsm --workflow ci.yml --commit "$(git rev-parse HEAD)" --status completed --json headSha,status,conclusion,url --limit 1`, followed by recorded `gh run view ... --json jobs` | pending |
