...
3. Recommend diagnostic JSON keys `topology.final_states`, `topology.transitions[*].mode` with string values `internal`/`external`, and `analysis.reachability.non_final_sinks`. Keep `analysis.reachability.terminal` as the existing topological sink list. The string mode is directly intelligible when a self edge is external; keep `from`, `to`, `priority`, `has_guard`, and all existing keys. These names are a Phase 31 design recommendation under discretion, not existing schema values. [VERIFIED: `src/fast_fsm/visualization.py:399-479`; locked D-02/D-05 and discretion in `31-CONTEXT.md:18,23,31-33`]
...
1. **How far should mode metadata extend through compatibility adjacency matrices?** Existing JSON contains a canonical `topology.transitions` row and a separate sparse adjacency projection; dense validator matrices and supplied-matrix validation have exact row shape. Recommendation: require explicit mode on canonical diagnostic JSON transition rows, and decide whether to add it to sparse/dense rows only with coordinated matrix validation and compatibility tests. [VERIFIED: `src/fast_fsm/visualization.py:426-454,586-652`; `src/fast_fsm/_diagnostics.py:438-532`; locked D-05 in `31-CONTEXT.md:23`]
2. **Which state counts as known-final for an uncommitted rejection?** The current state is always canonical at an owned trigger boundary, while no destination is committed for rejection. Recommendation: emit source/current finality only if the field name makes that meaning explicit; never emit destination finality for uncommitted rejection. [VERIFIED: `src/fast_fsm/core.py:3932-3956,4450-4492,5572-5603`; locked D-03/D-06 in `31-CONTEXT.md:19,24`]
