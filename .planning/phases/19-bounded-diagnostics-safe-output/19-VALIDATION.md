...
> Pre-execution validation contract for correct, bounded, snapshot-consistent
> diagnostics, grammar-safe output, and ownership-safe redacted logging.

---

## Validation Posture

This document is a plan-time strategy, not execution evidence. Every row is
pending until its owning plan task lands and the listed command passes. Frontmatter
must remain `status: draft`, `nyquist_compliant: false`, and
`wave_0_complete: false` until `$gsd-validate-phase` or equivalent independent
post-implementation audit verifies the finished candidate.
...
|---------|------------------|--------------|-------------------------|-----------|-------------------|-------------|--------|
| 19-00-01 | Wave 0 harness | DIAG-01–08, TEST-07 | Strict-RED graph fixtures, exact budget ledger assertions, snapshot-call spy, duplicate/empty schemas | unit/property scaffold | `uv run pytest tests/test_diagnostic_contracts.py -x -q` | No — Wave 0 | pending |
| 19-00-02 | Wave 0 harness | OUT-01–05 | Hostile renderer corpus, capture-handler leak oracle, redactor failure oracle, handler ownership fixtures | security/integration scaffold | `uv run pytest tests/test_output_safety.py tests/test_logging_config.py -x -q` | `test_output_safety.py`: no; logging file: extend | pending |
| 19-00-03 | Wave 0 harness | DIAG-08, OUT-03–05, TEST-07 | Phase 19 inventory and asserted pure/fresh-compiled runner without native-shadow reuse | structural/conformance | `uv run python tools/phase16_isolated_verify.py --suite phase19` | Harness exists; suite missing | pending |
| 19-01-01 | Plan 01 / tracer | DIAG-01, DIAG-07, DIAG-08 | One synchronized deeply stable snapshot, one shared deterministic budget, declared-initial reachability, explicit exhaustion | unit + integration | `uv run pytest tests/test_diagnostic_contracts.py tests/test_graph_invariants.py -x -q -k 'snapshot or initial or budget'` | Wave 0 prerequisite | pending |
| 19-02-01 | Plan 02 / graph | DIAG-04, DIAG-05 | Complete self-loop/long-cycle SCC membership and memoized condensation-DAG depth | unit/property | `uv run pytest tests/test_diagnostic_contracts.py -x -q -k 'cycle or scc or depth or longest'` | Wave 0 prerequisite | pending |
| 19-02-02 | Plan 02 / graph | DIAG-06, DIAG-07, TEST-07 | Sparse-first results, dense preflight before allocation, path count/length/expansion bounds | unit/adversarial | `uv run pytest tests/test_diagnostic_contracts.py -x -q -k 'sparse or dense or path or budget'` | Wave 0 prerequisite | pending |
| 19-03-01 | Plan 03 / API | DIAG-02, DIAG-03, DIAG-07, DIAG-08 | Duplicate names preserve position/cardinality/order; zero comparison has exact `None` aggregates; each input captured once | schema/integration | `uv run pytest tests/test_diagnostic_contracts.py tests/test_validation.py -x -q -k 'compare or batch or duplicate or empty or snapshot'` | Wave 0 prerequisite + legacy file | pending |
| 19-03-02 | Plan 03 / API | DIAG-01, DIAG-04–08 | Validators, reports, and JSON share sparse analysis/status without silent partial fallback | integration/regression | `uv run pytest tests/test_diagnostic_contracts.py tests/test_validation.py tests/test_visualization.py -x -q -k 'json or report or status or exhaust or snapshot'` | Wave 0 prerequisite + legacy files | pending |
| 19-04-01 | Plan 04 / output | OUT-01, OUT-02, DIAG-08 | Opaque snapshot-order IDs and separate Mermaid/PlantUML/Markdown encoders keep hostile text inert and byte-stable | security/property/golden | `uv run pytest tests/test_output_safety.py tests/test_visualization.py -x -q -k 'opaque or mermaid or plantuml or markdown or snapshot or stable'` | Wave 0 prerequisite + legacy file | pending |
| 19-05-01 | Plan 05 / logging | OUT-03, OUT-04, TEST-07 | Disabled trace stays constant-time/allocation-free; default metadata has no payload; explicit redactor receives minimum input and fails closed | security/performance | `uv run pytest tests/test_logging_config.py tests/test_performance_benchmarks.py -x -q -k 'trace or redact or payload or trigger_min_throughput'` | Extend existing files | pending |
| 19-05-02 | Plan 05 / logging | OUT-05 | Application handlers/filters/formatters survive; only marked library handler changes; propagation is explicit; handles restore idempotently/out of order | logging integration | `uv run pytest tests/test_logging_config.py -x -q -k 'handler or restore or propagation or delegate'` | Extend existing file | pending |
| 19-06-01 | Plan 06 / contracts | DIAG-01–08, OUT-01–05, TEST-07 | Public docs, ADR, SPRs, exports, signatures, complexity and budget semantics agree | docs/structural/type | `task typecheck-mypy && task typecheck-ty && uv run sphinx-build -b html docs docs/_build/html -W --keep-going && uv run sphinx-build -b doctest docs docs/_build/doctest` | Existing infrastructure | pending |
| 19-06-02 | Plan 06 / phase gate | DIAG-01–08, OUT-01–05, TEST-07 | Asserted pure and fresh native parity, slots, O(1) runtime boundary, compiled throughput floor, full regression suite | conformance/performance/release | `uv run python tools/phase16_isolated_verify.py --suite phase19` | Wave 0 suite prerequisite | pending |
...
| Check | Current state |
|-------|---------------|
| Every requirement maps to focused and final evidence | ready in this strategy |
| Every provisional task has automated verification | ready in this strategy |
| No three consecutive unverified tasks | enforced; currently zero unverified rows |
| Wave 0 test files/harness extensions exist | pending implementation |
| Strict-RED failures observed before production edits | pending implementation |
| Pure and compiled semantic origins asserted | pending Phase 19 harness execution |
| Performance/complexity gates pass | pending implementation |
| Full suite and baseline freshness pass | pending final gate |
| Independent Nyquist audit performed | pending post-execution audit |

## Validation Sign-Off

| Criterion | Status |
|-----------|--------|
| All tasks have automated verification or Wave 0 dependencies | strategy complete; execution pending |
| Sampling continuity has no three consecutive unverified tasks | strategy complete; planner must preserve |
| Wave 0 covers every missing reference | defined; not yet complete |
| No watch-mode flags or timing sleeps | strategy complete |
| Focused feedback target is under 30 seconds | target set; measure during execution |
| Asserted pure/fresh-compiled parity passes | pending |
| Deterministic budget boundaries pass | pending |
| Hostile grammar/logging matrix passes | pending |
| Compiled trigger remains at least 200,000 operations/second | pending |
| `nyquist_compliant: true` | intentionally not set before audit |

**Approval:** pending implementation and independent validation audit.
